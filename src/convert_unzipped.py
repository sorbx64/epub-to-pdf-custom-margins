...
def create_parser():
	parser = argparse.ArgumentParser(description="Command line unzipped epub to pdf converter. output in same folder as unzipped folder is located with '--unzipped-no-final-margins' added to end of pdf.")
	parser.add_argument("-i", "--input", required=True, metavar="UNZIPPED_FOLDER", help="path to input epub file")
	return parser
...
def main():
	...
	if os.path.exists(args.input):
		...
		opf_docnames = []
		epub_book = epub.read_epub('{}.epub'.format(extraction_dir))
